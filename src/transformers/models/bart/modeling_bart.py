...
class BartEncoderLayerSource3(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.source_attn = BartAttention(
            embed_dim=self.embed_dim,
            num_heads=config.encoder_attention_heads,
            dropout=config.attention_dropout,
        )
    ...
    def forward(
        ...
        if source_out != None:
            residual = hidden_states
            hidden_states1, attn_weights1, _ = self.self_attn(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                layer_head_mask=layer_head_mask,
                output_attentions=output_attentions,
            ...
            #print(hidden_states1.shape, hidden_states2.shape)
        ...
        if output_attentions:
            outputs += (attn_weights1,)

        return outputs

    def get_ratio(self):
        if self.source_encoder_dropout:
            frand = float(uniform(0, 1))
            if self.source_encoder_mixup and self.training:
                return [frand, 1 - frand]
            if frand < self.source_encoder_dropout_ratio and self.training:
                return [1, 0]
            elif frand > 1 - self.source_encoder_dropout_ratio and self.training:
                return [0, 1]
            else:
                return [0.5, 0.5]
        else:
            return [self.encoder_ratio, self.source_encoder_ratio]
...
class BartDecoderLayerSource3(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            # add cross-attn to positions 3,4 of present_key_value tuple
            present_key_value = present_key_value + cross_attn_present_key_value1
...
class BartDecoderSource4(BartPretrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`BartDecoderLayer`]
...
class BartModelSource0(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
...
class BartModelSource1(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
...
class BartModelSource2(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
...
class BartModelSource4(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        ...
        self.decoder = BartDecoderSource4(config, self.shared)
...
class BartForConditionalGenerationSource0(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource0(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
...
class BartForConditionalGenerationSource1(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource1(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
...
class BartForConditionalGenerationSource2(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource2(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
...
class BartForConditionalGenerationSource4(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource4(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
    ...
    def get_fusion_model(self):
        return self.model.get_fusion_model()
