...
class BartEncoderSource3a(BartPretrainedModel):
    ...
    def forward(
        ...
        if source_mask is not None:
            source_mask = _expand_mask(source_mask, inputs_embeds.dtype)
...
class BartDecoderSource3a(BartPretrainedModel):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            ...
            encoder_attention_mask = _expand_mask(encoder_attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1])
            if source_mask is not None:
                source_mask = _expand_mask(source_mask, inputs_embeds.dtype, tgt_len=input_shape[-1])
...
class BartModelSource2a(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
...
class BartModelSource2b(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
...
class BartForConditionalGenerationSource2a(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource2a(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
...
class BartForConditionalGenerationSource2b(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelSource2b(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
...
class BartForConditionalGenerationSource3b(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]
